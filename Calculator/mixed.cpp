...
{
    int sign = number.get_num() < 0 ? -1:1;
    if(number.get_num() == 0 || number.get_denom() == 1)
        out<<number.get_num();
    else if (number.get_num() / number.get_denom() != 0)
    {

        out<< sign * number.get_num() / number.get_denom() << " " <<
              sign * (number.get_num() % number.get_denom()) <<"/"<<number.get_denom();
    }
    else
    {
