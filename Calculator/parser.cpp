...
void Parser::exponent()
{
    ...
    if (q_temp->front()->b && *(string*)q_temp->front()->v == "(")
    {
        (this->*pedmas[q_temp->front()->b])();
        q->enqueue(s_operators->pop());
    }
    else //if (!q_temp->front()->b) // mixed object
    {
        q->enqueue(q_temp->dequeue());
        q->enqueue(s_operators->pop());
    }
...
void Parser::createToken(char *t)
{
    ...
    if (ispunct(t[0]) && t[1] == '\0')
    {
        ...
        *s_d = (string)t;
        ...
        // checks if it's a negative paranthesis
        if (!q_temp->empty() && q_temp->back()->b && *(string*)q_temp->back()->v == "-")// && *(string*)tw->v == "(")
        {

            q_temp->dequeue_behind();

            if (!q_temp->back()->b)
            {
                cout << "neg\n";
//                700 ^ ( 2 ) + 234.234 + 1 4/5 - - ( 234 * 3 )
                twin *mult0 = new twin;
...
//                q_temp->enqueue(negOne);
                q_temp->enqueue(mult0);
            }

            mixed *m = new mixed(-1,0,1);

            twin *negOne = new twin;
            twin *mult = new twin;
...
void Parser::printQueueTemp()
{
    Node<twin*> *ptr = q_temp->getHead();

    cout << "Printing temporary queue\n";

    for(; ptr; ptr = ptr->nextNode())
    {

        // calling a function pointer depending if it's bool false/true (i.e. mixed/operator)
        (this->*pt[ptr->getData()->b])(ptr);
    }
...
void Parser::twin_true_temp(Node<twin*> *ptr)
{
    cout << (*(string*)ptr->getData()->v)[0] << " ";
}
